...
func RunMigrations() {
	...
	if err != nil {
		logger.Log.Error("Failed to connect to UserService database for migrations",
	...
	wd, _ := os.Getwd()
	migrationsPath := fmt.Sprintf("file://%s/user-service/db/migrations", filepath.Join(wd))
	...
	logger.Log.Info("UserService migrations applied successfully")
}
